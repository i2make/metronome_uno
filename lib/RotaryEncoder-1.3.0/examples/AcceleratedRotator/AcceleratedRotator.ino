 ...
 * * Select a long cuttoff. If two encoderBpm rotations happen longer apart than the cutoff,
 *   they are not accelerated anymore. Without the long cutoff, not moving the encoderBpm for a while
 ...
 *   not really a problem, it is unrealistic to achieve (how quickly can you tick the encoderBpm?) and
 ...
 *   * An encoderBpm with 24 increments per 360 degrees
 *   * A useful range of 0..1000 rotations
 *   * Making a 180 degree rotation (12 increments) on the encoderBpm within 50ms will hit
 ...
 *   controller and application, you might not be able to sample the encoderBpm quickly enough to achieve this.
 *//*
...
void loop()
{
  ...
  if (pos != newPos) {
    ...
    RotaryEncoder::Direction currentDirection = encoderBpm.getDirection();
    if (currentDirection == lastMovementDirection && 
      ...
      unsigned long deltat = encoderBpm.getMillisBetweenRotations();
      ...
      if (deltat < kAccelerationLongCutoffMillis) {
        ...
        // Round by adding 1
        // Then again remove 1 to determine the actual delta to the encoderBpm
        // value, as the encoderBpm already ticked by 1 tick in the correct
